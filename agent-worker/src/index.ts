 ...
 */
async function runSession(_userPrompt) {
  ...
  try {
    const session = await client.createSession({
      ...
      ...(COPILOT_REASONING_EFFORT && COPILOT_REASONING_EFFORT !== 'none' ? { reasoningEffort: COPILOT_REASONING_EFFORT as 'low' | 'medium' | 'high' | 'xhigh' } : {}),
 ...
 */
function collectCommits(baseSha: string, cwd?: string) {
  ...
  for (const record of records) {
    ...
    if (files.length === 0) {
      ...
      } catch { /* ignore, leave files empty */ }
    }
 ...
 */
function squashIntoBaseIfNeeded(baseSha: string, cwd?: string) {
  ...
  session.on('tool.execution_complete', (e) => {
    ...
    for (const [id, startTime] of Object.entries(toolTimers)) {
      if (id.startsWith(toolName + '_')) {
        durationMs = Date.now() - (startTime as number);
...
async function main() {
  ...
  const baseSha = git(['rev-parse', 'HEAD'], undefined)?.toString().trim();
  ...
  try {
    ...
    const rootHeadSha = git(['rev-parse', 'HEAD'], undefined)?.toString().trim();
    ...
    if (hasAgentCommits) {
      ...
      let rootCommits = hasRootCommits ? collectCommits(baseSha, undefined) : [];
      ...
      if (validation.valid) {
        ...
        if (TASK_ID) {
          if (hasRootCommits) {
            ...
            const squashResult = squashIntoBaseIfNeeded(baseSha, undefined);
      ...
      const statusOutput = git(['status', '--porcelain'], undefined);
