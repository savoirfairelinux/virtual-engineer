...
import { CURSOR_PROVIDER } from './cursor.js';
import type { AgentProviderDefinition } from './types.js';

const AGENT_PROVIDERS = new Map<string, AgentProviderDefinition>(
  [COPILOT_PROVIDER, CLAUDE_PROVIDER, AIDER_PROVIDER, GOOSE_PROVIDER, CODEX_PROVIDER, GEMINI_PROVIDER, OPENCODE_PROVIDER, CURSOR_PROVIDER].map((provider) => [provider.id, provider])
);
