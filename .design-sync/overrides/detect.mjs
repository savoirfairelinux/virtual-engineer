// forked from design-sync lib/detect.mjs — the repo root walk hits
// data/openshell-gateway-state/{pki,openshell,.local} (0700, root-owned
// runtime PKI state for a local gateway container) and crashes with EACCES;
// upstream's ls() has no permission guard. Skip unreadable dirs instead.
//
...
export function findStorybookDirs(root, depth = 4) {
  ...
  (function walk(d, lvl) {
    ...
    try {
      ...
      if (err.code === 'EACCES' || err.code === 'EPERM') return; // unreadable dir — not ours to open, skip it
      throw err;
    }
