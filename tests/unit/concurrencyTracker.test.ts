...
describe("ConcurrencyTracker", () => {
  it("acquire increments all 3 counters and release decrements", async () => {
    ...
    const lease = await t.acquire(pid("p1"), aid("a1"));
  ...
  it("project ownership does not gate starts", async () => {
    ...
    const t = createConcurrencyTracker(stubs.deps);
    expect(await t.acquire(pid("p1"), aid("a1"))).not.toBeNull();
    expect(await t.acquire(pid("p1"), aid("a2"))).not.toBeNull();
  });
  ...
  it("acquire returns false when per-agent limit reached and does not mutate", async () => {
    ...
    expect(await t.acquire(pid("p2"), aid("a1"))).toBeNull();
  ...
  it("acquire returns false when integration limit reached", async () => {
    ...
    const t = createConcurrencyTracker(stubs.deps);
    expect(await t.acquire(pid("p1"), aid("a1"))).not.toBeNull();
    expect(await t.acquire(pid("p2"), aid("a2"))).not.toBeNull();
    expect(await t.acquire(pid("p3"), aid("a3"))).toBeNull();
  });
  ...
  it("counts multiple projects and agents for diagnostics", async () => {
    ...
    expect(await t.acquire(pid("p1"), aid("a1"))).not.toBeNull();
    expect(await t.acquire(pid("p1"), aid("a2"))).not.toBeNull();
    expect(await t.acquire(pid("p1"), aid("a3"))).not.toBeNull();
    expect(await t.acquire(pid("p2"), aid("a3"))).not.toBeNull(); // p2 still empty
    const s = t.snapshot();
  ...
  it("treats agent maxConcurrent <= 0 as 1", async () => {
    ...
    const t = createConcurrencyTracker(stubs.deps);
    expect(await t.acquire(pid("p1"), aid("a1"))).not.toBeNull();
    expect(await t.acquire(pid("p1"), aid("a1"))).toBeNull(); // floor=1 enforced
  });
  ...
  it("releases the exact acquired integration after agent configuration changes", async () => {
    const stubs = makeStubs({ cacheTtlMs: 0 });
    const t = createConcurrencyTracker(stubs.deps);
    stubs.agents.set("a1", { id: aid("a1"), maxConcurrent: 2, integrationId: "copilot-old" });
    const oldLease = await t.acquire(pid("p1"), aid("a1"));
    stubs.agents.set("a1", { id: aid("a1"), maxConcurrent: 2, integrationId: "copilot-new" });
    const newLease = await t.acquire(pid("p1"), aid("a1"));

    expect(t.snapshot().perAgent).toEqual({ "copilot-old": 1, "copilot-new": 1 });
    t.release(oldLease!);
    expect(t.snapshot().perAgent).toEqual({ "copilot-new": 1 });
    t.release(newLease!);
    expect(t.snapshot()).toEqual({ global: 0, perProject: {}, perAgent: {} });
  });

  it("consumes a lease only once", async () => {
    const t = createConcurrencyTracker(makeStubs({ perAgent: 2 }).deps);
    const lease = await t.acquire(pid("p1"), aid("a1"));
    t.release(lease!);
    t.release(lease!);
    expect(t.snapshot()).toEqual({ global: 0, perProject: {}, perAgent: {} });
  ...
  describe("cache TTL", () => {
    ...
    it("agent limit change is reflected on next acquire after TTL elapses", async () => {
      ...
      const t = createConcurrencyTracker(stubs.deps);
      const firstLease = await t.acquire(pid("p1"), aid("a1"));
      expect(firstLease).not.toBeNull();
      // Even after release, cache still says perAgent=1
      t.release(firstLease!);
      // Bump the limit at the source
      stubs.agents.set("a1", { id: aid("a1"), maxConcurrent: 2 });
      // Within TTL: new value not yet visible.
      expect(await t.acquire(pid("p1"), aid("a1"))).not.toBeNull();
      expect(await t.acquire(pid("p2"), aid("a1"))).toBeNull(); // cache still says 1
      vi.setSystemTime(new Date(Date.now() + 2000));
      expect(await t.acquire(pid("p2"), aid("a1"))).not.toBeNull(); // cache refreshed → limit=2
    });
  });
});
