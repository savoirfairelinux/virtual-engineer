...
function createMigrationLedger(raw: Database.Database, rows: readonly MigrationLedgerRow[]): void {
  raw.exec(`
  ...
  `);
  const insertRow = raw.prepare(
    "INSERT INTO __drizzle_migrations (hash, created_at) VALUES (?, ?)"
  );
  for (const row of rows) insertRow.run(row.hash, row.created_at);
}
...
/** Strip every tracked migration after the frozen bridge so the fixture looks ledgerless-legacy. */
function removePostBridgeSchema(raw: Database.Database): void {
  raw.exec(`
...
describe("runDatabaseMigrations", () => {
  it("applies the tracked baseline to an empty database", async () => {
    ...
    try {
      ...
      expect(rows).toEqual(await trackedLedgerRows());
    } finally {
      raw.close();
    }
  });

  it("does not add another ledger row on a second run", async () => {
    const raw = openMemoryDatabase();
    try {
      runDatabaseMigrations(raw);
      runDatabaseMigrations(raw);
      ...
      expect(row.count).toBe((await trackedLedgerRows()).length);
    } finally {
      raw.close();
    }
  });
  ...
  it("rejects a forged matching ledger without the canonical schema", async () => {
    const raw = openMemoryDatabase();
    try {
      createMigrationLedger(raw, await trackedLedgerRows());
  ...
  it("upgrades and adopts a recognized projects-only legacy database", async () => {
    ...
    try {
      ...
      expect(ledger).toEqual(await trackedLedgerRows());
    } finally {
      raw.close();
    }
  });
  ...
  it("installs canonical triggers after creating the ledger during legacy adoption", async () => {
    ...
    try {
      ...
      const ledgerRows = await trackedLedgerRows();
      migrationModule.adoptLegacyDatabaseForTest(
        raw,
        canonical,
        ledgerRows.map((row) => ({ hash: row.hash, folderMillis: row.created_at }))
      );
