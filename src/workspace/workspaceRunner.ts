...
export interface WorkspaceRunnerConfig {
  ...
  /** Docker network used by agent containers. */
...
export class DockerWorkspaceRunner implements WorkspaceRunner {
  private agentAdapter: AgentAdapter | undefined;
  ...
  async runAgentInDocker(
    ...
    const { volumeName, homeVolumeName } = requireDockerContextVolumes(context);
    log.info(
    ...
    const networkMode = spec.networkMode ?? this.config.networkMode;
    if (networkMode) {
  ...
  async runAgent(handle: WorkspaceHandle, context: TaskContext, adapter?: AgentAdapter): Promise<AgentResult> {
    ...
    if (!resolvedAdapter) {
      throw new Error(`No agent adapter configured for task ${handle.taskId}`);
    }
    log.info(
  ...
  async cloneRepo(
    ...
    try {
      const { volumeName } = requireDockerWorkspaceVolumes(handle);
      log.info(
        { taskId: handle.taskId, repoUrl, branch, volume: volumeName },
  ...
  async runReviewInDocker(
    ...
    const { volumeName, homeVolumeName } = requireDockerWorkspaceVolumes(handle);
    ...
    const adapter = input.agentAdapter ?? this.agentAdapter;
    if (!adapter) {
      throw new Error(`No agent adapter configured for review task ${handle.taskId}`);
    }
  ...
  async destroyWorkspace(handle: WorkspaceHandle): Promise<void> {
    const { volumeName, homeVolumeName } = requireDockerWorkspaceVolumes(handle);
    log.info({ taskId: handle.taskId, volumeName, homeVolumeName }, "destroying workspace volumes");
    // Stop any containers still using the workspace volume (e.g. after an agent timeout)
    await stopContainersUsingVolume(volumeName);
    try {
    ...
    try {
      await removeVolume(homeVolumeName);
    } catch {
...
function requireDockerContextVolumes(context: TaskContext): {
  volumeName: string;
  homeVolumeName: string;
} {
  if (!context.volumeName || !context.homeVolumeName) {
    throw new Error(`Legacy Docker execution requires workspace volumes for task ${context.taskId}`);
  }
  return {
...
function requireDockerWorkspaceVolumes(handle: WorkspaceHandle): {
  volumeName: string;
  homeVolumeName: string;
} {
  if (!handle.volumeName || !handle.homeVolumeName) {
    throw new Error(`Legacy Docker workspace is missing volumes for task ${handle.taskId}`);
  }
  return {
