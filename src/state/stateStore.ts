...
export class SqliteStateStore {
  ...
  private applyMigrations(): void {
    ...
    this.dropColumnIfExists("projects", "skill_discovery_enabled");
    this.dropColumnIfExists("projects", "local_skills_path");
  ...
  private dropColumnIfExists(tableName: string, columnName: string): void {
    ...
    const quotedColumn = `\`${columnName.replaceAll("`", "``")}\``;
    this.raw.exec(`ALTER TABLE ${quotedTable} DROP COLUMN ${quotedColumn}`);
  }
