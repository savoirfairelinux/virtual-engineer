...
function adoptLegacyDatabase(
  ...
  if (migrations.length === 0) throw new Error("Tracked Drizzle baseline is missing");
  ...
  try {
    raw.transaction(() => {
      ...
      for (const migration of migrations) insertLedgerRow.run(migration.hash, migration.folderMillis);
...
function ensureLegacyColumns(raw: Database.Database): void {
  const additions: ReadonlyArray<readonly [string, string, string]> = [
    ...
    ["app_settings", "ticket_close_max_retries", "INTEGER"],
    ["app_settings", "ticket_close_retry_min_timeout_ms", "INTEGER"],
  ];
 ...
 * on large instances.
 */
function backfillLegacyCycleCosts(raw: Database.Database): void {
