...
export function createPromptStore(context: PromptStoreContext): PromptStoreApi {
  ...
  async function loadDefaultPrompts(): Promise<Array<{ id: string; label: string; promptType: "system" | "user"; content: string }>> {
  ...
  async function createPrompt(label: string, content: string): Promise<Prompt> {
