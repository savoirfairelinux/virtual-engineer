...
export function validateSkillSourceSshAuth(source: SkillSourceDiscoveryInput): Promise<void> {
  // Not async: validation failures must surface as a rejected promise, not a
  // synchronous throw, for await/`.catch()` callers.
  try {
    if (!isSshSkillSource(source)) return Promise.resolve();
