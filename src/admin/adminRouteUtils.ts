 ...
 * Writes a 501 and returns `false` when missing; narrows `store` to `T` (via the type predicate)
 * when present. Assign the checked expression to a local `const` first if it's used inside a
 * closure — narrowing on a dotted property access does not survive crossing a closure boundary.
 */
export function requireStore<T>(store: T | undefined, res: ServerResponse, message: string): store is T {
  if (!store) {
