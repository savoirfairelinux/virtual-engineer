...
/**
 * Resolve the externally-reachable base URL of the admin dashboard.
 *
 * Prefers the operator-configured `publicBaseUrl`; otherwise falls back to the
 * admin server's own host/port (useful for local single-host deployments).
 */
export function resolvePublicBaseUrl(opts: {
  ...
  const configured = opts.publicBaseUrl?.trim();
