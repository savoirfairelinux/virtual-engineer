...
- **Secret transport**: agent credentials are injected only at `openshell sandbox create --env`, becoming Pod environment variables. Never repeat secrets on `sandbox exec --env`, because OpenShell includes the resulting environment prefix in gateway command-preview logs; exec-time env is limited to non-secret values such as `USER_PROMPT_FILE`.
