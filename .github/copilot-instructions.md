...
- For each agent cycle the host creates an ephemeral working directory, clones the repo host-side via `HostGitExecutor`, then runs the agent in a uniquely named **OpenShell sandbox** (`virtual-engineer-workspace:latest`) using an **upload → exec → download** lifecycle: the workspace (incl. `.git`) is uploaded into the sandbox, the agent edits files and may create one or more local commits, and the result is downloaded back to the host. Before any host Git command, VE rejects symlinked Git metadata and rebuilds `.git/config`, hooks, attributes, and alternates from host-trusted remote data. The **host owns all git plumbing** (clone/checkout/cherry-pick/**push**) and review-system credentials through `src/vcs/`. Every host-side git invocation is hardened (`trustedGitArgs`/`trustedGitEnv` — `core.hooksPath`, `include.path`, and `GIT_CONFIG_GLOBAL`/`GIT_CONFIG_SYSTEM` pinned to `/dev/null`), applied both by `HostGitExecutor` and by `NodeGitRunner`. `pushProjectChanges` only runs git in repositories the runner reports through `listTrustedRepoPaths()`; a push target whose clone failed is agent-authored and is refused rather than handed credentials. Authenticated HTTPS clone URLs are immediately replaced by credential-free remotes before upload. Admin-configured post-clone scripts run inside the policy-constrained sandbox, never on the host. Push credentials never enter the sandbox. The sandbox and working directory are destroyed on exit. There are **no** Docker named volumes, helper containers, or `DockerWorkspaceRunner`.
- **Sandbox spec** (built by `buildCodegenContainerSpec` / `buildReviewContainerSpec` in `src/agents/containerSpecBuilders.ts`, consumed by `copilotAdapter.ts`, `claudeAdapter.ts`, `aiderAdapter.ts`, and `gooseAdapter.ts`): the spec is exactly `{ image, env, command, userPromptContent?, egress? }` — there is no `networkMode`, no `additionalDockerArgs`, and no `--read-only` / `--cap-drop` / `--security-opt` / `--tmpfs`. Sandbox isolation and hardening come from the OpenShell gateway + runtime **policies** (deny-by-default network/filesystem/process/inference; writable paths are only `/sandbox`, `/tmp`, `/dev/null`; `run_as_user`/`run_as_group` = `sandbox`), not Docker flags; the declared `egress` is opened per run with `allowEgress`. VE streams the `openshell sandbox exec` stdout/stderr pipes into the existing adapter/review callbacks, so `__ve_event` frames reach `agentLogBus` and the admin SSE UI during execution while complete buffers remain available for final parsing. `AGENT_TIMEOUT_MS` is also passed to OpenShell's `sandbox exec --timeout`. Sandbox paths: repository (and exec `workdir`) at `/sandbox/<workspace-dir>` — the basename of the uploaded host workspace (`<taskId>-<mkdtemp suffix>`), with each project push target under its own `localPath` inside it — HOME `/sandbox`, prompt file `/tmp/user-prompt.txt`, worker runtime `/app/agent-worker/`, MCP submission artifact `/tmp/ve-agent-submission.json`; `/workspace` and `/ve-home` no longer exist. The agent image (`Dockerfile.agent`) pre-installs Node 24, git, gh, — for the `aider` provider — Python 3 + the Aider CLI (via `uv tool install aider-chat`, symlinked onto `/usr/local/bin/aider`), and — for the `goose` provider — the Goose CLI (via the official `download_cli.sh` installer with a pinned `GOOSE_VERSION`, installed to `/usr/local/bin/goose`). Repository behavior is provider-native: VE does not configure a local skill path, scan manifests, or expose an enable switch. Copilot uses `enableConfigDiscovery=true` (native repository skills plus repository MCP configuration), Claude uses project/user settings with all native skills and `strictMcpConfig=true`, Aider receives no VE-injected repository manifests, and Goose keeps its native repository behavior.
...
- **OpenShell deployment/auth**: OpenShell is pinned to 0.0.83 and remains the sole application runtime. `scripts/start.sh` uses the gateway's Docker compute driver by default (Docker-outside-of-Docker with a same-path state bind); `OPENSHELL_COMPUTE_DRIVER=kubernetes` selects the experimental k3s/Helm path. Docker mode generates persistent gateway-JWT keys with the official `generate-certs` command, publishes client access on host loopback, and publishes the callback port separately on the private `openshell-docker` bridge. The agent image must contain a `sandbox` user/group with `/sandbox` as its home. Do not restore a direct Docker workspace runner. The Helm chart and upstream gateway/supervisor images use verified OCI digests. Production VE images must be private GHCR `@sha256` references with pull secrets in both `virtual-engineer` and `ve-agents`. CLI/user access uses the named `virtual-engineer` Keycloak OIDC profile with anonymous access disabled. On explicit authentication failure, `OpenShellClient` coalesces a client-credentials login and replays the command once; the client secret is environment-only. Sandbox supervisors keep their separate gateway-minted JWT identity.
- **Providers (per capability)**: issue_tracking = Redmine | GitLab Issues | GitHub Issues; code_review / source_control = Gerrit | GitLab Merge Requests | GitHub Pull Requests; agent_execution = Copilot | Claude | Aider | Goose | Codex | Cursor | Mock. Provider credentials live on `integrations`, while GitLab project selection is VE-project-owned (`project_integration_bindings` issue_tracking `{ ticketProjectKey }`, `project_push_targets.repoKey`, code_review `{ repos }` bindings).
...
src/
  ...
  plugins/              # registry, pluginManager, init, descriptors/{index,github,
                        # gitlab,gerrit,redmine,copilot,claude,aider,goose,codex,mock}.ts (unified
...
agent-worker/src/       # TS worker inside the agent sandbox: index.ts
                        ...
                        # {types,events,copilot,claude,aider,goose,codex,cursor,registry}.ts
...
- `integrations` (TEXT `id` PK): `provider`, `name`, `config_json`, `enabled` (INTEGER), `discovered_resources_json`, `discovered_at`, timestamps. `provider` is one of `github | gitlab | gerrit | redmine | copilot | claude | aider | goose | codex | cursor | mock` (the former `type` column and the `category` concept were removed).
...
Like Codex/Goose, Cursor uses **MCP submission transport**: the runner writes `$HOME/.cursor/mcp.json` (global scope — not the project-scoped `.cursor/mcp.json` — so it never pollutes the agent's git status) registering the VE MCP submission server, so Cursor calls `ve_submit_changes` / `ve_submit_review` to deliver the structured result. Approval is handled by the global `--approve-mcps` CLI flag rather than a per-server trust field in the config file (unlike Gemini's `trust: true`). `--trust` grants headless workspace trust without an interactive prompt.
...
The Cursor CLI is installed in the agent image via its official installer (`curl https://cursor.com/install -fsS | bash`, see `Dockerfile.agent`), which symlinks the resulting binary to `~/.local/bin/{agent,cursor-agent}`; the Dockerfile re-symlinks it onto `/usr/local/bin/cursor-agent`. The installer has no documented version-pin option (unlike Goose's pinned release tag) — a reproducibility gap to revisit. Cursor's `~/.cursor/` config and credentials land under the sandbox HOME (`/sandbox`). Cursor needs outbound HTTPS to `api2.cursor.sh` (most API requests), `api5.cursor.sh` plus its `agent.*`/`agentn.*` subdomains (agent requests), and `repo42.cursor.sh` (codebase indexing) — a best-effort default egress list read from Cursor's published network-configuration docs, that should be verified against a live run with real credentials, since the sandbox's OpenShell **network runtime policy** denies all egress by default.
