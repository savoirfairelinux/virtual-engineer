...
- `projects.use_full_ticket_url_in_commits` (INTEGER, default `0`) is a **per-project, coding-only** toggle. When 1, the orchestrator computes a `ticketFooterLine` (via `formatTicketFooter(ticketId, ticketUrl, ticketSourceLabel, forceUrlFormat=true)`) and forwards it on `AgentSession.ticketFooterLine`; `copilotAdapter`/`claudeAdapter` inject it as `TICKET_FOOTER_LINE` env var. The agent-worker's `injectChangeIds()` (`agent-worker/src/commitUtils.ts`) appends this line to the trailer block of every agent commit alongside its Change-Id (idempotent — skipped if already present). This is the **only** mechanism that reaches Gerrit commit messages: `pushProjectChanges`'s own `appendTicketFooter`/`commitMsg` computation is dead code for all three VCS connectors because Gerrit, GitLab, and GitHub all implement `pushDirect` (which pushes the agent's HEAD commit(s) verbatim, bypassing the `vcsConnector.push(..., commitMsg, ...)` branch entirely).
