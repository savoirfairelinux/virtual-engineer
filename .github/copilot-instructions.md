...
- For each agent cycle the host creates an ephemeral working directory, clones the repo host-side via `HostGitExecutor`, then runs the agent in a uniquely named **OpenShell sandbox** (`virtual-engineer-workspace:latest`) using an **upload → exec → download** lifecycle: the workspace (incl. `.git`) is uploaded into the sandbox, the agent edits files and may create one or more local commits, and the result is downloaded back to the host. Before any host Git command, VE rejects symlinked Git metadata and rebuilds `.git/config`, hooks, attributes, and alternates from host-trusted remote data. The **host owns all git plumbing** (clone/checkout/cherry-pick/**push**) and review-system credentials through `src/vcs/`; authenticated HTTPS clone URLs are immediately replaced by credential-free remotes before upload. Admin-configured post-clone scripts run inside the policy-constrained sandbox, never on the host. Push credentials never enter the sandbox. The sandbox and working directory are destroyed on exit.
- **Sandbox spec** (built by `buildContainerSpec` / `buildReviewContainerSpec` in `src/agents/copilotAdapter.ts`): the OpenShell runner consumes only the spec's `image`, `env`, and `command`; sandbox isolation and hardening come from the OpenShell gateway + runtime **policies** (deny-by-default network/filesystem/process/inference), not Docker flags. VE streams the `openshell sandbox exec` stdout/stderr pipes into the existing adapter/review callbacks, so `__ve_event` frames reach `agentLogBus` and the admin SSE UI during execution while complete buffers remain available for final parsing. `AGENT_TIMEOUT_MS` is also passed to OpenShell's `sandbox exec --timeout`. The worker uses `/ve-home` for Copilot HOME (native modules) and an optional prompt file. When the project has `skillDiscoveryEnabled` set, both coding and review sandboxes receive `SKILL_DISCOVERY=1` and the worker loads `<repo>/.github/skills` (skills only; MCP discovery stays off; trusted repos only).
- **Secret transport**: supported agent credentials are removed from sandbox env and supplied through short-lived attached OpenShell providers. Provider creation uses bare `--credential KEY` arguments whose values exist only in the child process environment, keeping secrets out of argv, logs, labels, SQLite, and the parent environment. `sandbox create --env` and `sandbox exec --env` carry only non-secret values; exec-time env is limited to values such as `USER_PROMPT_FILE` because OpenShell includes that prefix in gateway command-preview logs.
...
- **OpenShell command output**: each host CLI process retains at most 32 MiB total across stdout/stderr while live callbacks continue. Overflow and abort terminate the detached process group with `SIGTERM`, then `SIGKILL` after two seconds if needed.
- **OpenShell deployment/auth**: OpenShell is pinned to 0.0.83; the Helm chart and upstream gateway/supervisor images use verified OCI digests. Production VE images must be private GHCR `@sha256` references with pull secrets in both `virtual-engineer` and `ve-agents`. CLI/user access uses the named `virtual-engineer` Keycloak OIDC profile with anonymous access disabled. On explicit authentication failure, `OpenShellClient` coalesces a client-credentials login and replays the command once; the client secret is environment-only. Sandbox supervisors keep their separate gateway-minted JWT identity.
...
2. **Docker review execution** — review tasks also run in the agent container (`REVIEW_MODE=1` via `workspaceRunner.runReviewInDocker`). `promptLoader.ts` decodes `SYSTEM_PROMPT_BASE64` (or reads single-line `SYSTEM_PROMPT`) and reads `USER_PROMPT_FILE` (`/ve-home/user-prompt.txt`), then emits `review.prompt_received` with lengths/sources only. Worker stdout is a JSON result envelope; `agentWorkerProtocol.ts` validates it at the workspace boundary and passes its exact `rawOutput` to the strict review parser without arbitrary unescaping. `src/review/copilotReviewAgent.ts` (host-side SDK client) is **legacy** — never instantiated in `src/`.
...
- **Concurrency gating is integration-scoped at agent-cycle time**: `ConcurrencyTracker` keys limits by `agents.integrationId` using `agents.maxConcurrent`. It no longer gates `PollingLoop` or `startTaskForProject`; it gates active `runAgentCycle` executions (Docker-heavy phase). `acquire()` returns an opaque single-use lease that captures the exact integration counter, and cycle cleanup releases that lease without re-reading mutable agent configuration.
