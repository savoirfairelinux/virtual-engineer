...
- **Sandbox spec** (built by `buildContainerSpec` / `buildReviewContainerSpec` in `src/agents/containerSpecBuilders.ts`, consumed by `copilotAdapter.ts`, `claudeAdapter.ts`, `aiderAdapter.ts`, and `gooseAdapter.ts`): the OpenShell runner consumes only the spec's `image`, `env`, and `command`; sandbox isolation and hardening come from the OpenShell gateway + runtime **policies** (deny-by-default network/filesystem/process/inference), not Docker flags. VE streams the `openshell sandbox exec` stdout/stderr pipes into the existing adapter/review callbacks, so `__ve_event` frames reach `agentLogBus` and the admin SSE UI during execution while complete buffers remain available for final parsing. `AGENT_TIMEOUT_MS` is also passed to OpenShell's `sandbox exec --timeout`. The worker uses `/ve-home` for agent HOME (native modules) and an optional prompt file. The agent image (`Dockerfile.agent`) pre-installs Node 24, git, gh, — for the `aider` provider — Python 3 + the Aider CLI (via `uv tool install aider-chat`, symlinked onto `/usr/local/bin/aider`), and — for the `goose` provider — the Goose CLI (via the official `download_cli.sh` installer with a pinned `GOOSE_VERSION`, installed to `/usr/local/bin/goose`). Repository behavior is provider-native: VE does not configure a local skill path, scan manifests, or expose an enable switch. Copilot uses `enableConfigDiscovery=true` (native repository skills plus repository MCP configuration), Claude uses project/user settings with all native skills and `strictMcpConfig=true`, Aider receives no VE-injected repository manifests, and Goose keeps its native repository behavior.
...
- **Providers (per capability)**: issue_tracking = Redmine | GitLab Issues | GitHub Issues; code_review / source_control = Gerrit | GitLab Merge Requests | GitHub Pull Requests; agent_execution = Copilot | Claude | Aider | Goose | Mock. Provider credentials live on `integrations`, while GitLab project selection is VE-project-owned (`project_integration_bindings` issue_tracking `{ ticketProjectKey }`, `project_push_targets.repoKey`, code_review `{ repos }` bindings).
...
agent-worker/src/       # TS worker inside the agent sandbox: index.ts
                        ...
                        # {types,events,copilot,claude,aider,goose,registry}.ts
...
The Goose CLI is installed in the agent image via the official `download_cli.sh` installer with a pinned `GOOSE_VERSION` (see `Dockerfile.agent`); the binary lands in `/usr/local/bin/goose`. Goose's `~/.config/goose/` config lands under `/ve-home` inside the sandbox. Goose needs outbound HTTPS to the upstream LLM API (and HTTP to Ollama); the sandbox's OpenShell **network runtime policy** must allow that egress explicitly, since the default posture is deny-all.
...
- **Container image rebuild**: after editing `src/agents/copilotAdapter.ts`, `src/agents/claudeAdapter.ts`, `src/agents/aiderAdapter.ts`, `src/agents/gooseAdapter.ts`, `agent-worker/src/**`, or `Dockerfile.agent`, run `docker build -f Dockerfile.agent -t virtual-engineer-workspace:latest .` and restart `npm run dev`.
