...
The `aider` provider runs the **Aider** CLI (https://aider.chat, a Python package that wraps any LLM backend via litellm) inside the same agent container. The host `AiderAdapter` (`src/agents/aiderAdapter.ts`) injects `AGENT_PROVIDER=aider`, the selected backend's litellm auth env var(s), and `AIDER_MODEL` **only when a model is configured** (otherwise the Aider CLI picks its own default — no hardcoded default in VE). The Aider runner (`agent-worker/src/providers/aider.ts`, resolved by the worker's provider registry when `AGENT_PROVIDER=aider`) spawns `aider --message-file <prompt> --yes --no-pretty --no-stream --commit-prompt <conventional-commits>` as a subprocess against `/workspace` and maps its streamed output onto the shared `__ve_event` / commit / `AgentResult` pipeline. Both coding and review flows are supported (review uses `REVIEW_MODE=1` with `--chat-mode ask --no-auto-commits --no-dirty-commits`; git stays enabled so Aider can build its repo-map).
