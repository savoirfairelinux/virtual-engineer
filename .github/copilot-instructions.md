...
agent-worker/src/       # TS worker inside the agent container: index.ts
                        ...
                        # {types,events,copilot,claude,aider,registry}.ts (per-provider
...
- `agents.system_prompt_id` and `agents.instructions_prompt_id` are required by the admin API and agent store for every new agent and cannot be cleared. The nullable DB columns only preserve compatibility with legacy rows; runtime resolution is fail-closed and never substitutes generic or review-integration descriptor prompts.
