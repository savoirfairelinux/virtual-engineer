...
- Built-in prompt IDs remain provider-specific for compatibility and per-ID overrides, but their defaults come from five shared files: `system_generic_code.md`, `instructions_generic_code.md`, `instructions_feedback_code.md`, `system_review.md`, and `instructions_review.md`. System prompts own durable role/safety/commit boundaries; instructions own workflow only. Review output shape is never stored in these defaults: MCP supplies the provider-specific JSON Schema for Copilot/Claude, while Aider receives the corresponding text contract at runtime.
...
- `projects.skill_discovery_enabled` (INTEGER, default `0`) is a **per-project** trust gate for local repository skills only. When 1, the orchestrator forwards it on `AgentSession.skillDiscoveryEnabled` (coding) or `ReviewWorkspaceInput.skillDiscoveryEnabled` (review), and the respective `buildContainerSpec` / `buildReviewContainerSpec` injects `SKILL_DISCOVERY=1` so the in-container agent loads local skills from `local_skills_path`. There is no longer an `AGENT_SKILL_DISCOVERY_ENABLED` env flag.
- `projects.local_skills_path` (TEXT, default `.github/skills`) stores the workspace-relative local skills directory. The admin UI exposes it in the `Local Project Skills` section with an `Enable` checkbox and `path` field; the worker falls back to `.github/skills` if an invalid path reaches the container and emits one `skills.local_loaded` timeline event with the configured path and sorted local skill names.
- `projects.skill_sources_json` (TEXT JSON, default `'[]'`) stores external skill sources as `[{ source, skills, installAll?, sshUser?, sshPort?, sshKeyPath?, sshKnownHostsPath? }]`. Remote sources are installed whenever configured, independent of `skill_discovery_enabled`; the workspace runner installs sources with `npx skills` into `/ve-home` before starting the agent. Empty `skills` is valid only with `installAll: true`; `sshUser`/`sshPort`/`sshKeyPath` are optional per-source SSH connection hints, and `sshKnownHostsPath` enables strict SSH host key verification. The database/API default remains empty, but the admin UI's new-project form preloads `ssh://g1.sfl.io/sfl/agent-skills` on port `29419` with `installAll: true`; saving the untouched form persists that source. The UI can list available skills dynamically via `npx skills add -l`; presets must not hardcode selected skill names.
...
Copilot preserves its native CLI system foundation by sending the selected Agent Instructions with explicit SDK `systemMessage.mode = "append"`. Its descriptor exposes native reasoning effort through `modelConfig.providerOptions.reasoningEffort`. Coding and review sessions receive one explicit worker-owned stdio MCP server (`ve_submit_changes` or `ve_submit_review`); `enableConfigDiscovery` remains false, so repository MCP files are not loaded. Review sessions approve repository reads and `ve_submit_review` only; shell, write, URL, and unrelated tool requests are rejected. The worker requires exactly one provider-observed MCP submission call and a schema-valid artifact; Copilot CLI reports that call as `ve-submission-<tool>`, while Claude reports `mcp__ve-submission__<tool>`. The MCP call records a typed completion intent only; the host still owns validation, push/review effects, and state transitions.
