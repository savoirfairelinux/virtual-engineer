...
- For each agent cycle the host creates an ephemeral working directory, clones the repo host-side via `HostGitExecutor`, then runs the agent in an **OpenShell sandbox** (`virtual-engineer-workspace:latest`) using an **upload → exec → download** lifecycle: the workspace (incl. `.git`) is uploaded into the sandbox, the agent edits files and may create one or more local commits, and the result is downloaded back to the host. The **host owns all git plumbing** (clone/checkout/cherry-pick/**push**) and review-system credentials through `src/vcs/`; authenticated HTTPS clone URLs are immediately replaced by credential-free remotes before upload. Push credentials never enter the sandbox. The sandbox and working directory are destroyed on exit.
...
- **Runtime policy tables** are separate from PBAC: `runtime_policies` stores validated OpenShell policy YAML by protection kind; `runtime_policy_bindings` assigns a runtime policy to exactly one project or agent. Project bindings override agent bindings. Effective policies compose at most one binding per protection kind (`filesystem`, `network`, `process`, `inference`); duplicate bindings of the same kind fail closed. `policy_denial_events` records scrubbed sandbox denials. Runtime store: `src/state/stores/runtimePolicyStore.ts`; resolver: `src/openshell/runtimePolicyResolver.ts`. Never reuse PBAC's `policy_bindings` for sandbox assignments.
...
Worker `sendAndWait` timeout ≈ 540s. `AGENT_TIMEOUT_MS` (default 60 min) bounds both the host orchestration and the OpenShell remote exec (`--timeout`, rounded up to seconds).
