...
- **Concurrency at 3 levels**: `app_concurrency.global`, `projects.maxConcurrent`, `agents.maxConcurrent`. The in-memory `ConcurrencyTracker` (`src/orchestrator/concurrencyTracker.ts`) gates `Orchestrator.startTaskForProject` and is consulted by `PollingLoop` per tick.
