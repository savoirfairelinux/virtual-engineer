...
- **Runtime policy tables** are separate from PBAC: `runtime_policies` stores OpenShell policy YAML by protection kind; `runtime_policy_bindings` assigns a runtime policy to exactly one project or agent; `policy_denial_events` records scrubbed sandbox denials. Runtime store: `src/state/stores/runtimePolicyStore.ts`. Never reuse PBAC's `policy_bindings` for sandbox assignments.
