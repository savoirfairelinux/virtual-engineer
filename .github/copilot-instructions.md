...
- **Providers (per capability)**: issue_tracking = Redmine | GitLab Issues | GitHub Issues; code_review / source_control = Gerrit | GitLab Merge Requests | GitHub Pull Requests; agent_execution = Copilot | Claude | Aider | Goose | Codex | Gemini CLI | Mock. Provider credentials live on `integrations`, while GitLab project selection is VE-project-owned (`project_integration_bindings` issue_tracking `{ ticketProjectKey }`, `project_push_targets.repoKey`, code_review `{ repos }` bindings).
...
src/
  ...
  plugins/              # registry, pluginManager, init, descriptors/{index,github,
                        # gitlab,gerrit,redmine,copilot,claude,aider,goose,codex,gemini,mock}.ts
...
agent-worker/src/       # TS worker inside the agent sandbox: index.ts
                        ...
                        # {types,events,copilot,claude,aider,goose,codex,gemini,registry}.ts
...
Like Codex/Goose, Gemini uses **MCP submission transport**: the runner writes `$HOME/.gemini/settings.json` (user scope, sibling to the repo directory so it never pollutes git status) registering the VE MCP submission server (`/app/agent-worker/dist/mcpSubmissionServer.js`) as a `mcpServers.ve-submission` stdio entry with `"trust": true` (bypasses tool confirmation for that server regardless of approval mode), so Gemini calls `ve_submit_changes` / `ve_submit_review` to deliver the structured result. `--skip-trust` grants session-only folder trust — required in headless mode, since Gemini's interactive trust dialog can't run and untrusted folders refuse to connect to any MCP server at all. The exact `tool_use`/`tool_result`/`result` JSONL event shapes are not fully documented upstream and should be verified against a live run before relying on them in production; stream `error` events are documented as "non-fatal warnings" and are logged rather than treated as fatal — the process exit code is the authoritative success/failure signal (0 success, 1 general error, 42 invalid prompt/arguments, 53 turn limit exceeded).
...
The Gemini CLI is installed in the agent image via `npm install -g @google/gemini-cli` (see `Dockerfile.agent`); the binary lands on the global npm bin directory (`/usr/local/bin/gemini`). Gemini's `~/.gemini/` config lands under the sandbox HOME (`/sandbox`). Gemini needs outbound HTTPS to `generativelanguage.googleapis.com` (Developer API and Vertex AI Express Mode calls) and `aiplatform.googleapis.com`/`oauth2.googleapis.com` (non-Express Vertex AI) — a best-effort default egress list that should be verified against a live run with real credentials, since the CLI may also contact update-check or telemetry hosts.
