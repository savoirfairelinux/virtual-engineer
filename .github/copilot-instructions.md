...
- **Container constraints** (set by `buildContainerSpec` in `src/agents/copilotAdapter.ts`): `--read-only` rootfs, `--cap-drop ALL`, `--security-opt no-new-privileges:true`, `--tmpfs /tmp:rw,nosuid,size=256m`, `/workspace` named-volume mount, `/ve-home` named-volume mount for Copilot HOME (native modules), optional `/ve-prompts` mount, `networkMode=virtual-engineer_ve-agent-net`. When the task's project has `skillDiscoveryEnabled` set (forwarded on `AgentSession.skillDiscoveryEnabled`) the env map also carries `SKILL_DISCOVERY=1`, and the worker loads `<repo>/.github/skills` (skills only; MCP discovery stays off; trusted repos only). Host-side review never loads skills.
...
- `projects.skill_discovery_enabled` (INTEGER, default `0`) is a **per-project, coding-only** toggle set in the project-setup form. When 1, the orchestrator forwards it on `AgentSession.skillDiscoveryEnabled` and `buildContainerSpec` injects `SKILL_DISCOVERY=1` so the in-container agent loads team skills from `<repo>/.github/skills`. There is no longer an `AGENT_SKILL_DISCOVERY_ENABLED` env flag; host-side review never loads skills.
