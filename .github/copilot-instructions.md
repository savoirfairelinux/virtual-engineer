...
- `projects.skill_sources_json` (TEXT JSON, default `'[]'`) stores remote skill sources as `[{ source, skills, installAll?, sshUser?, sshPort?, sshKeyPath?, sshKnownHostsPath? }]`. It is ignored unless `skill_discovery_enabled = 1`; when active, the worker fetches sources with `npx skills` into `/ve-home` before starting the agent. Empty `skills` is valid only with `installAll: true`; `sshUser`/`sshPort`/`sshKeyPath` are optional per-source SSH connection hints, and `sshKnownHostsPath` enables strict SSH host key verification. The admin UI can list available skills dynamically via `npx skills add -l`; presets must not hardcode selected skills.
