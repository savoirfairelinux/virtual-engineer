...
- **Providers (per capability)**: issue_tracking = Redmine | GitLab Issues | GitHub Issues; code_review / source_control = Gerrit | GitLab Merge Requests | GitHub Pull Requests; agent_execution = Copilot | Claude | Aider | Goose | Codex | OpenCode | Mock. Provider credentials live on `integrations`, while GitLab project selection is VE-project-owned (`project_integration_bindings` issue_tracking `{ ticketProjectKey }`, `project_push_targets.repoKey`, code_review `{ repos }` bindings).
...
agent-worker/src/       # TS worker inside the agent sandbox: index.ts
                        ...
                        # {types,events,copilot,claude,aider,goose,codex,registry}.ts
...
- `integrations` (TEXT `id` PK): `provider`, `name`, `config_json`, `enabled` (INTEGER), `discovered_resources_json`, `discovered_at`, timestamps. `provider` is one of `github | gitlab | gerrit | redmine | copilot | claude | aider | goose | mock` (the former `type` column and the `category` concept were removed).
...
Like Codex/Goose, OpenCode uses **MCP submission transport**: the runner writes a per-run `opencode.json` (via `OPENCODE_CONFIG`, isolated in a temp directory so concurrent/successive runs never share MCP registration or credentials) registering the VE MCP submission server (`/app/agent-worker/dist/mcpSubmissionServer.js`) as a `local` stdio MCP server, so OpenCode calls `ve_submit_changes` / `ve_submit_review` to deliver the structured result. The exact JSONL event shape from `opencode run --format json` is not fully documented upstream and should be verified against a live run before relying on it in production.
...
- `bedrock` → AWS env credential chain (mapped to `amazon-bedrock`; no key forwarded by VE)
- `perplexity` → `PERPLEXITY_API_KEY`
- `mistral` → `MISTRAL_API_KEY`
- `xai` → `XAI_API_KEY`
- `cerebras` → `CEREBRAS_API_KEY`
...
The OpenCode CLI is installed in the agent image via `npm install -g opencode-ai` (see `Dockerfile.agent`); the binary lands on the global npm bin directory (`/usr/local/bin/opencode`). OpenCode needs outbound HTTPS to the selected backend's upstream LLM API (and HTTP to Ollama); the sandbox's OpenShell **network runtime policy** must allow that egress explicitly, since the default posture is deny-all.
...
Cost: OpenCode has no AIU, so `agent_cycles` USD/credit columns stay null; token usage is still emitted as `assistant.usage` events (parsed from the JSONL usage event when present).

Skills: OpenCode natively reads `.opencode/skills/` (its own convention) plus the `.claude/skills/` and `.agents/skills/` compatibility paths; `skillSourceInstaller.ts` installs configured external skill sources into `.opencode/skills/` for this provider, alongside the existing Copilot/Claude/Goose targets.
