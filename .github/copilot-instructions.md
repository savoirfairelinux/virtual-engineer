...
- For each agent cycle the host creates Docker **named volumes**, clones the repo into a volume via a helper container, spawns an **ephemeral Docker container** (`virtual-engineer-workspace:latest`) that edits files and may create one or more local commits; push operations also run in helper containers against the volume. The **host still owns review-system credentials and push orchestration** through `src/vcs/`. Container and volumes are destroyed on exit; startup best-effort prunes unattached `ve-ws-*` / `ve-home-*` volumes orphaned by a previous process.
- **Container constraints** (set by `buildContainerSpec` / `buildReviewContainerSpec` in `src/agents/copilotAdapter.ts`, `claudeAdapter.ts`, `aiderAdapter.ts`, and `gooseAdapter.ts`): `--read-only` rootfs, `--cap-drop ALL`, `--security-opt no-new-privileges:true`, `--tmpfs /tmp:rw,nosuid,size=256m`, `/workspace` named-volume mount, `/ve-home` named-volume mount for agent HOME (native modules and global skills), optional `/ve-prompts` mount, `networkMode=virtual-engineer_ve-agent-net`. The agent image (`Dockerfile.agent`) pre-installs Node 24, git, gh, — for the `aider` provider — Python 3 + the Aider CLI (via `uv tool install aider-chat`, symlinked onto `/usr/local/bin/aider`), and — for the `goose` provider — the Goose CLI (via the official `download_cli.sh` installer with a pinned `GOOSE_VERSION`, installed to `/usr/local/bin/goose`). Repository behavior is provider-native: VE does not configure a local skill path, scan manifests, or expose an enable switch. Copilot uses `enableConfigDiscovery=true` (native repository skills plus repository MCP configuration), Claude uses project/user settings with all native skills and `strictMcpConfig=true`, and Aider receives no VE-injected repository manifests. Before the agent container starts, `src/workspace/workspaceRunner.ts` installs any project `skill_sources_json` external entries into the `/ve-home` volume with `npx --yes skills@1.5.16 add ... -g -a <agent> --copy -y`; the agent container receives only the installed skill files, never `SKILL_SOURCES_JSON`, `SSH_AUTH_SOCK`, `GIT_SSH_COMMAND`, or private-key paths. SSH skill sources can use the host `SSH_AUTH_SOCK` or a per-source `sshKeyPath` only in the short-lived helper install container, and can set `sshKnownHostsPath` to enforce strict host key checking. Configured SSH files must be regular files under `/app/secrets` or the repository `secrets/` directory; host reads use no-follow opens, descriptor checks, and same-descriptor reads. VE-generated SSH files live in a process-private `0700` temporary directory and are trusted only by exact process registration, not filename shape. Per-source `sshUser`/`sshPort` can complete Gerrit-style URLs that omit user/port. Remote sources are skills only and are supported for Copilot/Claude; Aider rejects them. Trust repositories and configured sources.
...
agent-worker/src/       # TS worker inside the agent container: index.ts
                        ...
                        # provider definitions + registry dispatch), commitUtils.ts,
...
- `projects.skill_sources_json` (TEXT JSON, default `'[]'`) stores optional external skill sources as `[{ source, skills, installAll?, sshUser?, sshPort?, sshKeyPath?, sshKnownHostsPath? }]`. The workspace runner installs configured sources with `npx skills` into `/ve-home` before starting the agent. Empty `skills` is valid only with `installAll: true`; `sshUser`/`sshPort`/`sshKeyPath` are optional per-source SSH connection hints, and `sshKnownHostsPath` enables strict SSH host key verification. The database/API default remains empty, but the admin UI's new-project form preloads `ssh://g1.sfl.io/sfl/agent-skills` on port `29419` with `installAll: true`; saving the untouched form persists that source. The UI can list available skills dynamically via `npx skills add -l`; presets must not hardcode selected skill names.
...
- `agents.model_config_json` → `providerOptions.toolAuthorization` carries per-agent tool authorization (blocklist-only, provider-specific shape). Claude/Copilot: `{ blockedTools: string[] }` (everything allowed by default; VE's network floor is immutable — user lists can only tighten). Aider: capability toggles (`suggestShellCommands`, `detectUrls`, `playwright`, `git`, `chatMode`). Goose: `{ developerExtension, gooseMode? }`. `allowedTools` is rejected by the admin API. The admin API validates per provider; host adapters forward as `CLAUDE_BLOCKED_TOOLS`/`COPILOT_BLOCKED_TOOLS` or `TOOL_AUTHORIZATION_JSON`. See `.github/context/modules/agents.md` § "Per-agent tool authorization".
...
| `SKILLS_CLI_PACKAGE` | `skills@1.5.16` | `npx` package used to list/install configured remote skill sources (read directly from `process.env`, not via `ConfigSchema`) |
...
The Goose CLI is installed in the agent image via the official `download_cli.sh` installer with a pinned `GOOSE_VERSION` (see `Dockerfile.agent`); the binary lands in `/usr/local/bin/goose`. Goose's `~/.config/goose/` config lands on the `/ve-home` named volume. Goose needs outbound HTTPS to the upstream LLM API (and HTTP to Ollama); the `virtual-engineer_ve-agent-net` Docker network must allow that egress, and Ollama-on-host needs `host.docker.internal` reachability.
