...
| Backups / recovery | `backupService`, `backupRestore`, `backupScheduler`, `backupDownloadTokenStore` |
...
- Backup tests cover online SQLite snapshot/session scrubbing, v2 HMAC authentication of database and prompt hashes, database/prompt tampering rejection, archive-entry limits with parser cancellation, forced replacement quarantine and restart idempotency, scheduler due-time/coalescing/retention behavior, PBAC-protected filename-scoped single-use download tokens with native streaming, independent schedule/inventory loading, schedule-form unsaved-navigation state, and launcher confirmation/force/read-only-mount contracts.
