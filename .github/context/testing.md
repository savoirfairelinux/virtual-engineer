...
| Plugins / runtime wiring | `pluginManager` (+ `.multiInstance`), `registry`, `runtimeBootstrap` (historical name; covers bootstrap wiring in `src/index.ts`), `runtimeRegistry`, `runtimeAwareWorkspaceRunner`, `openShellWorkspaceRunner`, `openshell`, `hostGitExecutor`, `runnerContract`, `adminRuntimeRoutes`, `integrationStreamEvents` |
