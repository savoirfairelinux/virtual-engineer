...
- Local skill tests cover the fixed `.github/skills` convention, rejection of environment overrides, canonical workspace containment, regular `SKILL.md` discovery, the single sorted `skills.local_loaded` event, Copilot native directories, Claude's temporary plugin and `Skill` tool, and Aider's repeated `--read` arguments.
