...
|---|---|
| Admin routes / server + UI | `adminServer` (+ `.behavior`, `.integration`), `adminUiSse`, `adminImageProxy`, `imageProxyTokenStore`, `adminHealthEndpoint`, `adminOverviewRoutes`, `adminPluginRoutes`, `adminPromptRoutes`, `adminAgentsRoutes`, `adminAgentsOAuthRoutes`, `adminProjectsRoutes` (+ `.relaunch`), `adminProjectStatisticsRoutes`, `adminConcurrencyRoutes`, `adminSettingsRoutes`, `adminIntegrationsDiscover`, `adminWebhookSecretRoutes`, `adminCostRoutes`, `adminModelUsageRoutes`, `adminAuthService`, `localPasswordAuthenticator`, `adminAuthRoutes`, `adminAuthSourceRoutes`, `authSourcesSection`, `adminServerRbac`, `adminPoliciesRoutes`, `adminRuntimePolicyRoutes`, `adminRuntimeStatusRoute`, `runtimePolicyValidation`, `adminAudit`, `adminAuditRoutes`, `commonPasswords`, `loginRateLimiter`, `closeAdminServer`, `dashboard` (+ `.configurationTab`), `agentFormModal`, `toolAuthorizationSection`, `toolUsageSummary`, `projectFormModal`, `projectStatisticsView`, `apiIdentityBoundary`, `appIdentityHandoff`, `configDirtyRace`, `configRouting`, `configPageSurface`, `configNavigation`, `configPermissions`, `configListFilters`, `configListSearch`, `guidedTour`, `topBar`, `identityReset`, `auditSection`, `auditExportSection` |
...
| State / stores | `taskDomain`, `stateMachine`, `stateStore` (+ `.projects`, `.cost`, `.reviewDedup`, `.modelUsage`), `projectStatisticsStore`, `settingsStore`, `databaseMigrations`, `migrations.projects`, `integrationStore`, `promptStore`, `runtimePolicyStore` (runtime policies/denials), `userStore`, `auditStore`, `pbacStores` |
| PBAC / authorization | `policyEngine`, `permissions`, `pbacStores`, `adminPoliciesRoutes`, `adminServerRbac` (owner/peer/group/admin isolation across projects, tasks, integrations, agents, prompts, OAuth apps; project-owner delegation; cross-owner reference and stream/aggregate leak regressions) |
| Admin authentication / LDAP | `ldapConfig`, `ldapDirectory`, `authSourceStore`, `adminAuthSourceRoutes`, `authSourcesSection` |
...
| Review runtime | `reviewOrchestrator` (including per-project manual/automatic assignment and no-backfill policy), `reviewRecovery`, `reviewRetriggerGuard`, `reviewStderrEvents`, `reviewPostingGate`, `reviewPromptBuilder`, `reviewOutputContract` (covered through parser/orchestrator suites), `reviewResultParser`, `reviewLiveLogs`, `liveLogFormat`, `liveLogWindow`, `agentCyclePresentation`, `commentHash`, `commentSeverity`, `revisionPatchset` |
...
- **LDAP exception:** directory suites run against a private, loopback-only OpenLDAP instance that the test starts itself. `tests/unit/helpers/ldapDirectoryServer.ts` creates a temporary directory with a throwaway CA and server certificate (`openssl`), a `slapd.conf` with the `memberof` overlay, and a base seed loaded by `slapadd`; it runs `slapd` on free LDAP/LDAPS ports, exposes `config()` / `addUser()` / `addGroup()` / `replaceAttribute()` / `deleteEntry()`, and stops and removes the instance afterwards. It uses the system `slapd` under `/`, or an extracted package when `VE_SLAPD_ROOT` is set (`apt-get download slapd && dpkg -x slapd_*.deb <dir>`; no root needed, and it avoids the system AppArmor profile). Suites use `describe.skipIf(!ldapAvailable)`; CI extracts `slapd` this way and sets `VE_REQUIRE_LDAP_TESTS=1`, which turns a missing `slapd` into a failure instead of a skip. `tests/unit/helpers/adminHttp.ts` starts a production-style admin server (`startAdminHttpServer`) with `requestJson` / `setupAdmin` / `login` helpers; jsdom UI suites forward relative SPA requests to it so the page is exercised against real routes and SQLite.
...
- Review orchestrator race tests must model patchset changes during agent execution and assert that the stale pass has no provider or posting-ledger side effects; a newer patchset requires a fresh checkout, diff, and agent run before posting. The execution deadline covers provider reads, abortable host-Git preparation, agent termination, freshness, posting/replies, and final status, but pauses while queued for capacity and restarts with a full window once a slot is acquired; lifecycle cancellation remains active while queued. Clone cleanup waits for termination, while a post timeout preserves ambiguous `REVIEW_COMMENTING`.
...
- Concurrency tracker tests retain every acquired lease and release that same lease. Lifecycle coordinator regressions cover pre-start cancellation, review posting cancellation, creation leases, and project tombstoning before deletion waits. Broader coverage includes overlapping acquisitions across an agent integration change, idempotent double release, abort-during-drain queue ordering, lifecycle-coordinated queue handoff and tracker-only queue timeout/cancellation, serialized poll/webhook/admin lifecycle operations, active-task execution-identity reconfiguration rejection (while idempotent full-form saves remain allowed), atomic parent/child project rollback, and setup failures before a code-generation cycle row exists.
- Project reconfiguration tests verify the 409 task summary, confirmation by the exact active task IDs, transaction-time detection of tasks added after the prompt, and UI re-confirmation with the refreshed list. They also assert that save itself leaves task states unchanged and that later source/repository incompatibilities persist a manual-retry failure rather than being auto-relaunched.
