...
- OpenShell client tests cover explicit profile/endpoint flags and lazy client-credentials renewal: an authentication failure triggers one shared login and one safe control-plane command replay, while direct endpoints never attempt profile login and `sandbox exec` stderr never triggers replay.
...
- Concurrency tracker tests retain every acquired lease and release that same lease. Regression coverage includes overlapping acquisitions across an agent integration change, idempotent double release, queued review acquisition/rejection, and setup failures before a code-generation cycle row exists.
