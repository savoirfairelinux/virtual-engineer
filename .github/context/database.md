...
- Backup archives include that SQLite snapshot and allowlisted prompt override Markdown files. Manifest format v2 uses HMAC-SHA256 with `ADMIN_AUTH_SECRET` to authenticate the canonical SQLite SHA-256 and deterministically code-unit-sorted prompt-override filename/hash inventory; the secret itself is never stored. Restore verifies the HMAC, database and prompt contents/inventory before running the normal tracked migration/schema validator and installation. Earlier v1 archives are rejected because their checksum and prompt inventory are not authenticated. The original secret is also required to decrypt restored provider credentials. Active sessions, OpenShell state, managed local OIDC files, and ephemeral workspaces are not included.
