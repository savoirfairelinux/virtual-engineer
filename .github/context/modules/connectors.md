...
The `provider` ids are `github | gitlab | gerrit | redmine | copilot | claude | aider | goose | codex | opencode | mock`. Repository push/commit lives in [src/vcs/](../../../src/vcs/) via `capabilities.source_control.createVcsConnector` — see [vcs.md](vcs.md). The `copilot`, `claude`, `aider`, `goose`, `codex`, `opencode`, and `mock` providers expose only `agent_execution` and have no connectors here.
