...
- optional `discoverModels(config)` hook for provider model choices, exposed by
  the read-scoped admin `POST /api/admin/integrations/:id/models/discover`
  endpoint
...
- optional `readWorkspaceManifestFiles(config, repoKey, revision?)` hook. GitHub and GitLab use authenticated recursive repository inventories plus content APIs with at most eight concurrent content reads; Gerrit performs a shallow, blob-filtered fetch and recursive tree listing in a temporary object store with the integration's SSH identity. Implementations return only allowlisted declarative files (standard manifests to four directory levels; contrib/CMake dependency files to eight outside generated trees), cap files at 200 and content at 256 KiB each, fail closed on truncated GitHub trees, and never return credentials. The workspace scan service owns parsing and bounded traversal through uniquely matched enabled `(integration, repository, revision)` gitlinks.
...
3. Every enabled integration row is instantiated and kept active in memory, including multiple rows of the same **provider**.
  ...
  rows from a removed provider) are retained in the database for inspection or
  deletion but are skipped at runtime and remain inactive. Admin create/update
  rejects unknown providers; there is no provider fallback.
...
5. Project-mode runtime paths may either reuse the active integration-global connector via `getConnectorForIntegration(integrationId)` or build a project-bound connector via `createConnectorForIntegration(integrationId, context)` / `createConnectorForCapability(integrationId, capability, context)` when the VE project owns part of the provider binding. Review status polling uses the repository prefix in `owner/repo#number` change ids to select an exact `repoKey`; it does not fall back to the first repository in a multi-repository binding.
...
1. Create `src/plugins/descriptors/<provider>.ts` declaring a `capabilities` map; implement the relevant capability factories (`issue_tracking.createConnector`, `code_review.createConnector` / `createReviewer`, `source_control.createVcsConnector`, or `agent_execution.buildAdapter`) plus `testConnection` and `getSummaryDetails`. Agent providers may also declare `agent_execution.configFields`.
