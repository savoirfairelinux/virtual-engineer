...
- One review deadline starts before provider details/diff, pauses without consuming the execution budget while waiting for agent capacity, and restarts with a full window once a slot is acquired. Lifecycle cancellation remains active while queued. The restarted deadline covers abortable workspace creation/clone/patch checkout, agent execution, freshness checks, comments/replies/vote, ledger persistence, and final provider status. Signal-aware workspace operations are awaited to termination before cleanup. A timeout after entering `REVIEW_COMMENTING` preserves that ambiguous state for restart recovery instead of claiming provider effects did not happen.
