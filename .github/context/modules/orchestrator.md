...
- `projectPushService.ts` performs trusted host-side multi-repository pushes,
	direct push calls, provider-native identity persistence, required-target
	outcomes, primary-task mirroring, and orphan-row tracking. It does
...
- webhook merge handling is dual-path: review tasks in `REVIEW_WATCHING` transition directly to `REVIEW_DONE`; code-generation webhooks update only the matching `change_per_repository` rows and transition through `MERGED -> CLOSING -> DONE` only after every active required review is merged
...
- project pushes always require `VcsConnector.pushDirect()` and preserve the worker-normalized agent commit chain. Gerrit uses `AgentResult.commits[]` for per-commit change tracking; GitLab/GitHub persist one canonical MR/PR per target.
- every configured push target is required. The push summary classifies each target as `PUSHED`, `EXISTING`, `NO_CHANGE`, `CLONE_FAILED`, or `PUSH_FAILED`; `EXISTING` preserves a prior review when that target has no new commits, while persisted successful rows retain the provider status (normally `OPEN`). Any failure prevents `IN_REVIEW`, and an all-`NO_CHANGE` result with no existing reviews follows the task's no-change abandonment path.
- after all targets succeed, the first pushed target by `commitOrder` is mirrored into the task-level external-change id/review URL so fallback polling can always rediscover the task
- the worker-owned `ve_submit_changes` MCP tool is a typed completion intent, not a push API. Commit collection, validation, provider-conditional Change-Id injection, `pushDirect()`, `change_per_repository` persistence, and state transitions still occur only after the agent container exits
...
- review feedback dispatch stops when `cycleCount >= maxAgentCycles`; reaching the configured cap never starts one extra cycle
...
- orphaned prior changes are marked `ORPHANED` and excluded from merge convergence. First-cycle/full-chain Gerrit pushes may orphan rows above the returned maximum index; retry-local partial updates preserve absent existing rows and match returned commits to their absolute indices by Change-Id, then subject hash.
...
- **multi-commit workspace restoration**: before reusing any persisted Change-Id, `checkoutPriorPatchsets()` restores every continuity-enabled Gerrit target in its own workspace subpath: commit 0 is checked out, then commits 1..N are cherry-picked in order. Missing checkout support or any restore failure stops the cycle before agent execution; VE never reuses a Change-Id against a fresh/partial clone.
...
- an agent-reported failed cycle records `RETRY_CYCLE`, then destroys its workspace and releases its concurrency slot before invoking the next cycle; this prevents same-task sandbox-name collisions and nested slot consumption
...
- **Change-Id continuity is per-push-target**: the orchestrator resolves every target connector before agent execution and records its continuity flag in the repository map. A branch-based root cannot suppress a Gerrit child's prior Change-Ids, and a Gerrit root cannot add Change-Ids to GitLab/GitHub children. `ROOT_CHANGE_ID` is derived only from the root repository's persisted index-0 row; the task-level primary mirror is used solely as a legacy fallback for single-target Gerrit tasks.
- **push refs are per-push-target**: each connector derives its own ref from the target branch and task identity. Only the primary target reuses `tasks.push_ref` for compatibility; a Gerrit ref is never forwarded to a GitLab/GitHub child, nor a feature branch to a Gerrit child. Identity repair uses the same rule.
