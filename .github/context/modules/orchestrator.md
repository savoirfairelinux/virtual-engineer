...
- fatal workflow handling re-reads the task before persisting an error; if an operator or external event already moved it to a terminal state such as `ABANDONED`, the handler preserves that state and failure reason instead of attempting an invalid transition to `FAILED`
