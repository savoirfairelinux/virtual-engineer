...
- One review deadline starts before provider details/diff, pauses without consuming the execution budget while waiting for agent capacity when lifecycle cancellation is available, and restarts with a full window once a slot is acquired. Without lifecycle coordination, the execution deadline remains the bounded queue-cancellation path. Lifecycle cancellation remains active while queued. The restarted deadline covers abortable workspace creation/clone/patch checkout, agent execution, freshness checks, comments/replies/vote, ledger persistence, and final provider status. Signal-aware workspace operations are awaited to termination before cleanup. A timeout after entering `REVIEW_COMMENTING` preserves that ambiguous state for restart recovery instead of claiming provider effects did not happen.
...
- Worker execution/protocol failures persist their bounded, secret-filtered diagnostics in the failed cycle's `metadata.workerOutput`, with stdout also available as `agentLogs`. No provider effects occur for these failures. Sandbox cleanup is unchanged; diagnostics remain in the cycle after cleanup. Older cycles without diagnostics cannot recover the discarded output.
...
- Saving an execution-affecting project change never fails or cancels its active tasks. Ticket tasks retain the originating ticket integration/project snapshot; before a ticket connector is used, the resolver compares that snapshot with the current binding. Review tasks require the original review integration and a still-configured repository, without falling back to another active integration. Code-generation changes retain their integration/repository identity; polling uses that persisted identity, and a retry cycle checks that every active change still has a matching push target before starting the agent. A concrete mismatch is persisted with the stable project-reconfiguration failure prefix and ends in `FAILED` or `REVIEW_FAILED`; the admin's automatic relaunch skips those failures so they remain available for manual retry.
...
- project agent runtime resolution reads credentials from the selected integration; managed OAuth envelopes remain stored until the provider adapter decrypts them, while API keys are materialized for direct use, and undecryptable configs fail closed
