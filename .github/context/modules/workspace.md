...
2. `prepareProjectWorkspace()` / `cloneRepo()` — clones each push target **on the host** (ordered by `commitOrder`; secondary-target failures are non-fatal), using bounded shallow-clone retries for transient network/pack failures and per-attempt timeouts, and cleaning partial destinations between attempts. A failed root clone clears its contents but preserves the workspace directory. It records a credential-free remote per local path in `trustedRemotes`.
