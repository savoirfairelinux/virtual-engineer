...
1. Legacy `createWorkspace()` creates `ve-ws-*` and `ve-home-*` named volumes; OpenShell `createWorkspace()` creates a host scratch directory and a uniquely named sandbox.
2. Legacy `prepareProjectWorkspace()` / `cloneRepo()` clones into `/workspace` inside helper containers. OpenShell clones on the host and records credential-free remotes.
3. Legacy `runAgentInDocker()` mounts the volumes at `/workspace` and `/ve-home`, then starts the adapter container on `AGENT_DOCKER_NETWORK`. OpenShell resolves policy/egress, uploads to `/sandbox`, and executes through the gateway.
4. Legacy VCS push operations run in short-lived helper containers against the named volume. OpenShell downloads the coding workspace and restores trusted Git metadata before host-side pushes; reviews upload only.
5. `destroyWorkspace()` removes the legacy volumes or the OpenShell sandbox and host directory.
