...
2. `prepareProjectWorkspace()` / `cloneRepo()` — clones every configured push target **on the host**, ordered by `commitOrder`, and records a credential-free remote per local path in `trustedRemotes`. Every target is required and uses its own enriched SSH key / `sshKnownHostsPath`; any clone failure aborts workspace preparation before the agent starts. Because this happens before `ProjectPushService`, the failed cycle/task is durable but no `CLONE_FAILED` `change_per_repository` row is written; that status remains a defensive push-service outcome when a supposedly cloned path is absent later.
