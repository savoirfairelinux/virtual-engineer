...
- `GET`/`PUT /api/admin/projects/:id/vendor-components` (`project.read` / `project.write`, scoped by `:id`) read and atomically replace the project's tracked vendor components. The payload accepts at most 500 entries; duplicate `sourcePath` values are rejected with 400, and writes emit the `project.vendor_components_update` audit action. An entry may declare the VE repository it is really maintained in through `integrationId` + `repoKey`; supplying only one of the two, or an `integrationId` that does not resolve to a stored integration, is rejected with 400. `POST /api/admin/projects` also accepts an optional `vendorComponents` array so a new coding project and its components are stored in one request — a failure rolls the project back rather than leaving it half-configured. Replacing components preserves each surviving entry's original `createdAt`.
- The scan preview labels every detected member with its `origin`. Members VE cannot push to expose a **Track** action that stages them as vendor components; `internal` members are not trackable, since they live in the repository and are edited directly. Tracked entries appear in a **Vendor Components** section with an editable note, a source-control integration picker and a repository key input, and are persisted alongside **Save** (only when the operator actually changed them). A half-filled binding is cleared before submission, so the operator can type the two fields in any order.
- Those same members also expose a **Push to…** repository picker listing every repository discovered on an enabled `source_control` integration. Choosing one adds a push target for the mapped repository, which covers a component whose declared URL is only a mirror (for example a BitBake `SRC_URI` pointing at GitHub while review happens on Gerrit). The clone URL comes from the **mapped** repository's discovered resources, never from the scanned URL, and the branch comes from provider discovery falling back to the repository default — the member's `revision` is the mirror's pinned commit and is deliberately never used as a branch. Mapping is deduplicated by `(integrationId, repoKey)` and derives the local path with the same rule as a manual target.
