...
- **Agents create/update**: `systemPromptId` and `instructionsPromptId` are mandatory, non-empty references to existing prompts with matching `system` / `instructions` roles. Creation without either prompt or with crossed roles returns 400; updates cannot clear them, and an old incomplete agent must supply both before any update succeeds.
