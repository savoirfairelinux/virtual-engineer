...
- Each member also exposes a secondary icon action, **Patch … locally**, for a component no repository of ours owns. It tracks the member as a vendor component instead of a push target, and its note tells the agent to patch it in place. Tracked entries appear in a **Vendor Components** section with an editable note, persisted alongside **Save** (only when the operator actually changed them).
