...
- **Prompts create**: `POST /prompts` requires `promptType = system | instructions`; there is no stored `user` prompt type. Built-in prompt IDs cannot be deleted, while custom prompts of either role can be.
