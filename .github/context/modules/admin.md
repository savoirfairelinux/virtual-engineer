...
- **Details masking**: `maskAuditDetails()` recursively replaces values of secret-bearing keys with `"***"`. Matching is **segment-based** (keys are split on camelCase / snake_case / kebab-case, and a segment must exactly equal `token`, `secret`, `password`, `passwd`, `pwd`, `credential(s)`, or `key`), so `sessionToken`/`webhookSecret`/`apiKey` are masked while benign identifiers like `monkey`, `keyboard`, and `publicKey` are not. Safelisted identifier keys (`repoKey`, `repoKeys`, `ticketProjectKey`, `publicKey`) and any key ending in `Path` (e.g. `sshKeyPath`, a filesystem path) are never masked. Cyclic object graphs are detected (`WeakSet` of visited objects) and resolve to `"[Circular]"` instead of recursing forever. Secrets are never written to the audit log.
