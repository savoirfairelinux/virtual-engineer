...
- Agent forms render advanced provider settings from `capabilities.agent_execution.configFields` and persist them in `modelConfig.providerOptions`; adding a provider does not require a provider-specific UI branch. The agent list labels the two required prompt roles as Agent Instructions and Workflow Instructions.
