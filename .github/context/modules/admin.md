...
- The scan preview labels every detected member with its `origin`. Members VE cannot push to expose a **Track** action that stages them as vendor components; `internal` members are not trackable, since they live in the repository and are edited directly. Tracked entries appear in a **Vendor Components** section with an editable note and are persisted alongside **Save** (only when the operator actually changed them).
