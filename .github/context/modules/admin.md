...
- The scan preview labels every detected member with its `origin`. Each member VE could not auto-resolve asks one question through a single **Where do changes go?** select: pick a repository \u2014 every repository discovered on an enabled `source_control` integration is listed \u2014 and the member becomes a push target the agent edits and VE pushes; pick **Patch locally** and it becomes a vendor component instead, whose note tells the agent to patch it in place. `internal` members ask nothing, since they live in the repository and are edited directly. Answering the question replaces the select with the resulting state, and tracked entries appear in a **Vendor Components** section with an editable note, persisted alongside **Save** (only when the operator actually changed them).
- Mapping a member to a repository covers a component whose declared URL is only a mirror (for example a BitBake `SRC_URI` pointing at GitHub while review happens on Gerrit). The clone URL comes from the **mapped** repository's discovered resources, never from the scanned URL, and the branch comes from provider discovery falling back to the repository default \u2014 the member's `revision` is the mirror's pinned commit and is deliberately never used as a branch. Mapping is deduplicated by `(integrationId, repoKey)` and derives the local path with the same rule as a manual target.
