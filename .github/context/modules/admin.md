...
| `adminProjectsRoutes.ts` | `/api/admin/projects` list/create, `/:id` get/update/delete, `enable`/`disable`. Registers `adminProjectWorkspaceRoutes.ts` and `adminProjectVendorComponentsRoutes.ts`. Ticket/review target validation, atomic push-target replacement, coding controls for Gerrit topic/ticket trailers/review links/CI retries, and automatic relaunch of FAILED/REVIEW_FAILED tasks on (re)configuration or re-enable. |
...
| Projects | `adminProjectsRoutes.ts` | CRUD `/projects`, `enable`/`disable`, `/skill-sources/list`, `/:id/skill-sources/list`, `GET`/`PUT /:id/vendor-components` | `project.read` / `project.write` / `project.delete` / `project.operate` (scoped, list-filtered) |
