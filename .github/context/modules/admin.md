...
- **Agents create/update**: `systemPromptId` and `instructionsPromptId` are mandatory, non-empty references to existing prompts. Creation without either prompt returns 400; updates cannot clear them, and an old incomplete agent must supply both before any update succeeds.
