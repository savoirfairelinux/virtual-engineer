...
- Copilot and Claude receive one explicit stdio MCP server owned by VE. Review sessions must call `ve_submit_review`; coding sessions must call `ve_submit_changes`. The server validates the exact JSON Schema, accepts one submission, and writes a mode-0600 artifact in `/ve-home`. The worker requires exactly one provider-observed submission call and revalidates the artifact before accepting it; it recognizes Copilot CLI's `ve-submission-<tool>` event name and Claude's `mcp__ve-submission__<tool>` name as the same VE tool. Copilot review sessions approve only reads whose requested and canonical paths remain under `/workspace`, rejecting traversal, missing paths, and symlink escapes; the VE review-submission tool is the only other allowed capability. Shell, writes, URLs, and unrelated tools are rejected. These tools submit intent only: they have no database, review-provider, push credential, or task-state access.
