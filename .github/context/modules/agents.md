...
- Claude keeps the native `claude_code` system preset and appends the selected Agent Instructions. Its descriptor exposes effort, thinking mode/budget, maximum turns, and maximum USD cost. Review mode supplies the immutable integration JSON Schema through the SDK's native `outputFormat` and consumes `structured_output`.
...
- Aider's per-session timeout terminates the CLI, rejects the cycle with an explicit timeout error, removes its temporary prompt directory, and emits `session.error`; a signal-driven null exit after timeout is never treated as success.
