...
1. **`package.json` `scripts`** changed → update [.github/context/testing.md](../context/testing.md), and update the **Quality Gates** section in [.github/copilot-instructions.md](../copilot-instructions.md) only when a gate command itself changes.
...
5. **`tsconfig.json`** strictness flags changed → update the **TypeScript Conventions** section in `copilot-instructions.md` and the strict-mode block in `skills/typescript-standard/SKILL.md`.
