...
description: "Keep configuration documentation in sync with ConfigSchema."
---
# Keep configuration docs in sync

When editing `src/config.ts`:

1. If a key was added to `ConfigSchema`, add it to the corresponding table in [.github/context/configuration.md](../context/configuration.md), which is the canonical environment reference.
2. If a `superRefine` rule was added/removed/changed, update the conditional-required notes in `configuration.md`.
3. If a default value changed, update `configuration.md`.
