...
1. If the sandbox spec changed, update the **Sandbox Environment** / engine-specific sections in [.github/context/modules/agents.md](../context/modules/agents.md), which is the canonical agent-execution reference. Update the **Non-negotiable Boundaries** section in [.github/copilot-instructions.md](../copilot-instructions.md) only when a sandbox, credential, or engine-inventory boundary changes. The supported engines are Copilot, Claude, Aider, Goose, Codex, Gemini CLI, OpenCode, and Cursor. Triggers:
...
2. If a new adapter file was added (alternative LLM backend, e.g. `claudeAdapter.ts` or `aiderAdapter.ts`), add it to the agent inventory in `modules/agents.md`, the provider ids in `modules/plugins.md` / `modules/connectors.md`, and the **Agents** section of `context/architecture.md`.
3. If the in-sandbox headless CLI boot, worker timeout, or review-mode (`REVIEW_MODE=1`) execution changed, update the affected provider execution section in `modules/agents.md`.
...
5. Provider ids are `github | gitlab | gerrit | redmine | copilot | claude | aider | goose | codex | gemini | opencode | cursor`. When adding an `agent_execution` engine, cover both its coding and review flows.
