...
6. Remote skill sources (`AgentSession.skillSourcesJson`) are **not installed by the worker** at HEAD. If you add an install path, remove the regression notes in `modules/agents.md`, `modules/workspace.md`, and `context/architecture.md`; until then, do not document skills as reaching the agent.
