...
All provider configuration lives in SQLite and is managed through the admin UI. The agent engine is pluggable: **Copilot**, **Claude**, **Aider**, **Goose**, **Codex**, **Gemini CLI**, or **Mock**.
...
- Rebuild the agent image after touching `src/agents/copilotAdapter.ts`, `src/agents/claudeAdapter.ts`, `src/agents/aiderAdapter.ts`, `src/agents/gooseAdapter.ts`, `src/agents/codexAdapter.ts`, `src/agents/geminiAdapter.ts`, `agent-worker/src/**`, or `Dockerfile.agent`:
