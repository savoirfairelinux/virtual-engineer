...
> **Switching Docker → Kubernetes requires manual cleanup first.** `start.sh` never stops the Docker driver's `ve-openshell-gateway` container when switching to `kubernetes`, so it keeps holding `OPENSHELL_GW_LOCAL_PORT` (30808 by default). The k3s `kubectl port-forward` then fails to bind that port and `start.sh` aborts with `Could not establish the OpenShell gateway tunnel.` Run `docker rm -f ve-openshell-gateway` before re-running `start.sh` with `OPENSHELL_COMPUTE_DRIVER=kubernetes`. The reverse switch (Kubernetes → Docker) is handled automatically: `start.sh` stops the managed `kubectl port-forward` before starting the Docker gateway container. Also avoid switching drivers while tasks are actively running — any sandbox still alive on the previous driver becomes unreachable once the CLI gateway profile points elsewhere, and its task will need to be cleaned up manually.
